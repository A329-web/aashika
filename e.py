...
print(
    "🖐️ Show fingers 1-5 to apply filters | ✌️ Show 2 fingers to take screenshot | ESC to exit"
)
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        # Switch filter based on finger count (if changed)
        if (
            finger_count < len(filters)
            and (time.time() - last_switch_time) > switch_delay
        ):
            if finger_count != current_filter:
                current_filter = finger_count
